...
parameters {
  ...
  real<lower=0>         phi ;
  # real                  Const_noise ;
  cholesky_factor_corr[C] L_b; 
  cholesky_factor_corr[C] L_r; 
}
transformed parameters {
  ...
  b_bias  <- (tau_b * L_b) * noise[1] ;
  b_resp  <- (tau_r * L_r) * noise[2] ;
}
model {
  // local variables
  ...
  for (n in 1:N) {
    ...
    ## theta_n <- inv_logit(fma(lnmajvavg[n], b_resp[congress[n]], b_bias[congress[n]] + Const)) ;
